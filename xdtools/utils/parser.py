...
from xdtools.style import ColorFill, GradientFill, PatternFill, ColorStroke, DropShadow, Blur, Font, Opacity, TextAttributes
...
def get_artwork_file_json(source):
    if source is not None:
        resources_file = source.read("artwork/pasteboard/graphics/graphicContent.agc")
        return json.loads(resources_file.decode('utf-8'))
        """os.listdir(resource_folder)"""
    else:
        raise Exception()
...
def get_public_file_json(source):
    """Return the public content JSON file of the provided source file"""
    if source is not None:
        public_file = source.read("public/card/artwork/pasteboard/graphics/graphicContent.agc")
        return json.loads(public_file.decode('utf-8'))
    else:
        raise Exception()

def show_file_contents(source):
    if source is not None:
...
def look_for_texts(x):
        for child in x:
            if child.type == 'text':
                print '{"' + child.name + '" : "' + child.raw_text + '"}'
...
def parse_artworks(source):
    """Return a list of Artworks that are in the provided source file."""
    artworks = []
    artwork_file_json = get_artwork_file_json(source)
    artwork_nodes = artwork_file_json['children']
    for artwork_node in artwork_nodes:
        artwork = parse_artwork(artwork_node, source)
